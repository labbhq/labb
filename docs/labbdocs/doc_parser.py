...
class DocParser:
    def __init__(
        ...
        Args:
            ...
            menu_order (str): "asc" (default) or "desc" to reverse the numeric-prefix
                order of menu entries. Blog-style sections list newest first.
        """
        self.name = name
    ...
    def create_menu_structure(self):
        ...
        def build_menu(current_path):
            ...
            reverse_order = self.menu_order == "desc"
            ...
            for item in sorted(current_path.iterdir(), reverse=reverse_order):
                if item.is_dir():
                    ...
                    menu_item = {"title": title, "path": url_path}
                    if icon := frontmatter_data.get("icon"):
                        menu_item["icon"] = icon
                    menu_items.append(menu_item)
    ...
    def save_to_yaml(self):
        ...
        header = (
            "# Auto-generated by `task docs:build`. Do not edit manually.\n"
            "# Source: docs/labbdocs/content/<section>/\n"
        )
