...
COMPONENT_CLASSES_FILE = (
    ".labb/labb-component-classes.txt"  # safelist data (@source'd by labb.css)
)
...
def _write_component_classes(
    config, resolved: Dict[str, "object"], output_path: str
) -> None:
...
def generate_css_artifacts(config, root: str = ".") -> None:
    ...
    if literals:
        lines += [
            "/* Tailwind @source globs (css.packages literals). */",
            *literals,
            "",
        ]
    lines += [
...
def _map_attributes_to_classes(
    ...
    for var_name, var_spec in variables.items():
        ...
        # Reactive ($sig), :bound, and {{ interpolated }} props all have a value
        # the scanner cannot resolve: emit every possible class.
        is_bound = (":" + var_name) in attributes
        is_reactive = isinstance(attr_value, str) and attr_value.startswith("$")
        is_template_var = isinstance(attr_value, str) and "{{" in attr_value
        if css_mapping and (is_bound or is_reactive or is_template_var):
